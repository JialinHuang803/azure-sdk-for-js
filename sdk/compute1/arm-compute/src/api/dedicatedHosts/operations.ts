...
import {
  ...
  computeDedicatedHostSerializer,
  computeDedicatedHostDeserializer,
  ComputeDedicatedHostUpdate,
  computeDedicatedHostUpdateSerializer,
  _ComputeDedicatedHostListResult,
  _computeDedicatedHostListResultDeserializer,
  _ComputeDedicatedHostSizeListResult,
  _computeDedicatedHostSizeListResultDeserializer,
...
export async function _listAvailableSizesDeserialize(
...
): Promise<_ComputeDedicatedHostSizeListResult> {
  ...
  return _computeDedicatedHostSizeListResultDeserializer(result.body);
}
...
export async function _listByHostGroupDeserialize(
  result: PathUncheckedResponse,
): Promise<_ComputeDedicatedHostListResult> {
  const expectedStatuses = ["200"];
  if (!expectedStatuses.includes(result.status)) {
    const error = createRestError(result);
    error.details = commonCloudErrorDeserializer(result.body);
  ...
  return _computeDedicatedHostListResultDeserializer(result.body);
}
...
export function listByHostGroup(
  ...
  hostGroupName: string,
  options: DedicatedHostsListByHostGroupOptionalParams = { requestOptions: {} },
): PagedAsyncIterableIterator<ComputeDedicatedHost> {
  return buildPagedAsyncIterator(
    context,
...
export function _updateSend(
  ...
  hostGroupName: string,
  hostName: string,
  parameters: ComputeDedicatedHostUpdate,
  ...
  return context
    ...
    .patch({
      ...
      headers: { accept: "application/json", ...options.requestOptions?.headers },
      body: computeDedicatedHostUpdateSerializer(parameters),
    });
}

export async function _updateDeserialize(
  result: PathUncheckedResponse,
): Promise<ComputeDedicatedHost> {
  const expectedStatuses = ["200", "201", "202"];
  if (!expectedStatuses.includes(result.status)) {
    const error = createRestError(result);
    error.details = commonCloudErrorDeserializer(result.body);
  ...
  return computeDedicatedHostDeserializer(result.body);
}
...
export function update(
  ...
  parameters: ComputeDedicatedHostUpdate,
  options: DedicatedHostsUpdateOptionalParams = { requestOptions: {} },
): PollerLike<OperationState<ComputeDedicatedHost>, ComputeDedicatedHost> {
  return getLongRunningPoller(context, _updateDeserialize, ["200", "201", "202"], {
    ...
    resourceLocationConfig: "location",
    apiVersion: "2025-04-01",
  }) as PollerLike<OperationState<ComputeDedicatedHost>, ComputeDedicatedHost>;
}
...
export function _createOrUpdateSend(
  ...
  hostGroupName: string,
  hostName: string,
  parameters: ComputeDedicatedHost,
  ...
  return context
    ...
    .put({
      ...
      contentType: "application/json",
      headers: { accept: "application/json", ...options.requestOptions?.headers },
      body: computeDedicatedHostSerializer(parameters),
    });
}
